import {getBlzClient, restartIpfsServerAndSwarm} from "@bluechip/testing/src/commonUtils";
import {generateContent} from "@bluechip/testing/src/fileUtils";
...
import {defaultSwarmConfig} from "@bluechip/testing";
...
const bluechipUrl = 'http://localhost:26667';
...
describe('storage module', function () {
    ...
    it.skip('hasContent should return true if content is pinned', () =>
        Promise.all(times(2).map(() =>
            ...
            .then(withCtxAwait('client', () => getBlzClient(bluechipUrl, mnemonic.getValue())))
    ...
    it('should query for v1 cids', () =>
        getBlzClient(bluechipUrl, mnemonic.getValue())
    ...
    it('should query a transaction by hash', () => {
        return getBlzClient(bluechipUrl, mnemonic.getValue())
            .then(bzSdk =>

...
import { defaultSwarmConfig, startSwarmWithClient } from '@bluechip/testing';
...
import { newBluechipClient } from '../../core';
import { newLocalWallet } from '../../wallets/localWallet';
import { VoteOption } from '../../bluechip/lib/generated/cosmos/gov/v1beta1/gov';
import delay from 'delay';
import { copyToContainer, getContainerId } from '@bluechip/testing/src/dockerUtils';
...
import { stopSwarm } from '@bluechip/testing/src/swarmUtils';
...
describe('gov module, local docker', function(){
      ...
      startSwarmWithClient()
        ...
        .then(passThroughAwait(() => {
          getContainerId('a.client.sentry')
            .then(id => copyToContainer(id, path.join(__dirname, './plans/do_nothing'), '/root/.bluechip/cosmovisor/upgrades'))
      ...
      startSwarmWithClient()
        ...
        .then(passThroughAwait(client => submitSoftwareUpgradeProposal(client.bzSdk, {
            ...
            plan: {
              name: 'double_supply',
              height: 25,
              info: 'double the supply of ubluechip',
            }
          }, {
            maxGas: 200_000,
            gasPrice: 10
        ...
        .then(passThroughAwait(() => {
          getContainerId('a.client.sentry')
            .then(id => copyToContainer(id, path.join(__dirname, './plans/double_supply'), '/root/.bluechip/cosmovisor/upgrades'))
          getContainerId('a.validator')
            .then(id => copyToContainer(id, path.join(__dirname, './plans/double_supply'), '/root/.bluechip/cosmovisor/upgrades'))
        }))
        .then(passThroughAwait(() => delay(10_000)))
  ...
  describe('community pool spend proposal', () => {
    ...
    it('should be able to vote on and pass a community pool spend proposal', () =>
      startSwarmWithClient()
        .then(passThroughAwait(ctx => fundCommunityPool(ctx.bzSdk, {
          amount: [{amount: 100_000_000, denom: 'ubluechip'}],
    ...
    it('should be able to receive funds from a community pool spend proposal', () =>
      startSwarmWithClient()
        .then(passThroughAwait(ctx => fundCommunityPool(ctx.bzSdk, {
          amount: [{amount: 500_000_000, denom: 'ubluechip'}],
          depositor: ctx.auth.address
        }, {maxGas: 200_000, gasPrice: 10})))
        .then(withCtxAwait("recipient", () => newBluechipClient({
...
describe.skip('gov module, local machine', () => {
  ...
  // Run a local bluechip instance
  // Set genesis.app_state.gov.deposit_params.max_deposit_period to "5s" in bluechip/config.yml
  // Set genesis.app_state.gov.voting_params.voting_period to "10s" in bluechip/config.yml
